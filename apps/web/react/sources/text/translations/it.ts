...
export const it: TranslationStructure = {
    ...
    acp: {
        ...
        content: {
            ...
            audioUnsupported: 'La riproduzione audio non e supportata',
            unknownType: ({ type }: { type: string }) => `Tipo di contenuto non supportato: ${type}`,
            terminal: 'Terminale',
            terminalRef: ({ id }: { id: string }) => `Sessione terminale: ${id}`,
            terminalCopied: 'Output del terminale copiato',
            diffCollapse: 'Mostra meno',
            diffExpand: ({ count }: { count: number }) => `Mostra altre ${count} righe`,
        },
        usage: {
            title: 'Finestra di contesto',
            tokens: ({ used, total }: { used: string; total: string }) => `${used} / ${total} token`,
        },
        commandPalette: {
            title: 'Comandi',
            searchPlaceholder: 'Cerca comandi...',
            emptyTitle: 'Nessun comando disponibile',
            emptyDescription: 'Questo agente non ha registrato alcun comando.',
            noResults: 'Nessun comando corrisponde alla ricerca',
            commandCount: ({ count }: { count: number }) => `${count}`,
        },
        config: {
            title: 'Configurazione',
            emptyTitle: 'Nessuna opzione di configurazione',
            emptyDescription: 'Questo agente non ha esposto opzioni configurabili.',
            applying: 'Applicazione...',
            readOnly: 'Sola lettura',
            unknownType: 'Tipo di opzione non supportato',
            selectPlaceholder: 'Seleziona un valore',
        },
        sessionBrowser: {
            title: 'Sessioni',
            emptyTitle: 'Nessuna sessione',
            emptyDescription: 'Nessuna sessione precedente trovata per questo agente.',
            active: 'Attiva',
            load: 'Carica',
            resume: 'Riprendi',
            fork: 'Dirama',
            switchSessionTitle: 'Cambiare sessione?',
            switchSessionMessage: 'Hai una sessione attiva. Il cambio lascerà la sessione corrente.',
            sessionCount: ({ count }: { count: number }) => count === 1 ? '1 sessione' : `${count} sessioni`,
        },
        agentPicker: {
            title: 'Agenti',
            active: 'Attivo',
            ...
            statusError: 'Errore',
            agentCount: ({ count }: { count: number }) => count === 1 ? '1 agente' : `${count} agenti`,
            switchTitle: 'Cambiare agente?',
            switchMessage: ({ name }: { name: string }) => `Passare a ${name}? Lo stato della sessione corrente verrà preservato.`,
            switchConfirm: 'Cambia',
            switchSuccess: ({ name }: { name: string }) => `Passato a ${name}`,
            switchFailed: ({ name }: { name: string }) => `Impossibile passare a ${name}. Ripristinato l'agente precedente.`,
            rollbackNotice: 'Il cambio di agente non è riuscito. Ripristinato l\'agente precedente.',
        },
        permission: {
            title: 'Permesso richiesto',
            expired: 'Questa richiesta di permesso e scaduta',
            rawInput: 'Mostra input grezzo',
            queueCount: ({ count }: { count: number }) => `${count} in attesa`,
            notificationTitle: 'Permesso richiesto',
            notificationBody: ({ tool }: { tool: string }) => `L'agente ha bisogno del permesso per eseguire: ${tool}`,
            historyTitle: 'Cronologia permessi',
            historyEmpty: 'Nessuna decisione sui permessi ancora',
            outcomeSelected: 'Approvato',
            outcomeExpired: 'Scaduto',
            outcomeCancelled: 'Annullato',
        },
    },
