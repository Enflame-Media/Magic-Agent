...
export const zhHans: TranslationStructure = {
    ...
    acp: {
        ...
        content: {
            ...
            audioUnsupported: '不支持音频播放',
            unknownType: ({ type }: { type: string }) => `不支持的内容类型: ${type}`,
            terminal: '终端',
            terminalRef: ({ id }: { id: string }) => `终端会话: ${id}`,
            terminalCopied: '终端输出已复制',
            diffCollapse: '收起',
            diffExpand: ({ count }: { count: number }) => `显示更多 ${count} 行`,
        },
        usage: {
            title: '上下文窗口',
            tokens: ({ used, total }: { used: string; total: string }) => `${used} / ${total} 令牌`,
        },
        commandPalette: {
            title: '命令',
            searchPlaceholder: '搜索命令...',
            emptyTitle: '无可用命令',
            emptyDescription: '此代理尚未注册任何命令。',
            noResults: '没有匹配的命令',
        ...
        config: {
            title: '配置',
            emptyTitle: '无配置选项',
            emptyDescription: '此代理未提供可配置选项。',
            applying: '应用中...',
            readOnly: '只读',
            unknownType: '不支持的选项类型',
            selectPlaceholder: '选择一个值',
        },
        sessionBrowser: {
            title: '会话',
            emptyTitle: '无会话',
            emptyDescription: '未找到此代理的历史会话。',
            active: '活跃',
            load: '加载',
            resume: '恢复',
            fork: '分支',
            switchSessionTitle: '切换会话？',
            switchSessionMessage: '您有一个活跃会话。切换将离开当前会话。',
        ...
        agentPicker: {
            ...
            switchTitle: '切换代理？',
            switchMessage: ({ name }: { name: string }) => `切换到 ${name}？当前会话状态将被保留。`,
            switchConfirm: '切换',
            switchSuccess: ({ name }: { name: string }) => `已切换到 ${name}`,
            switchFailed: ({ name }: { name: string }) => `切换到 ${name} 失败。已回滚到之前的代理。`,
            rollbackNotice: '代理切换失败。已回滚到之前的代理。',
        },
        permission: {
            title: '需要权限',
            expired: '此权限请求已过期',
            rawInput: '显示原始输入',
            queueCount: ({ count }: { count: number }) => `${count} 个待处理`,
            notificationTitle: '请求权限',
            notificationBody: ({ tool }: { tool: string }) => `代理需要权限来执行: ${tool}`,
            historyTitle: '权限历史',
            historyEmpty: '暂无权限决定',
            outcomeSelected: '已批准',
            outcomeExpired: '已过期',
