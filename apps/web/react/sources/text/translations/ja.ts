...
export const ja: TranslationStructure = {
    ...
    acp: {
        ...
        content: {
            ...
            audioUnsupported: 'オーディオ再生はサポートされていません',
            unknownType: ({ type }: { type: string }) => `サポートされていないコンテンツタイプ: ${type}`,
            terminal: 'ターミナル',
            terminalRef: ({ id }: { id: string }) => `ターミナルセッション: ${id}`,
            terminalCopied: 'ターミナル出力をコピーしました',
            diffCollapse: '折りたたむ',
            diffExpand: ({ count }: { count: number }) => `さらに${count}行を表示`,
        },
        usage: {
            title: 'コンテキストウィンドウ',
            tokens: ({ used, total }: { used: string; total: string }) => `${used} / ${total} トークン`,
        },
        commandPalette: {
            title: 'コマンド',
            searchPlaceholder: 'コマンドを検索...',
            emptyTitle: 'コマンドがありません',
            emptyDescription: 'このエージェントはコマンドを登録していません。',
            noResults: '検索に一致するコマンドがありません',
            commandCount: ({ count }: { count: number }) => `${count}`,
        },
        config: {
            title: '設定',
            emptyTitle: '設定オプションなし',
            emptyDescription: 'このエージェントは設定可能なオプションを公開していません。',
            applying: '適用中...',
            readOnly: '読み取り専用',
            unknownType: 'サポートされていないオプションタイプ',
            selectPlaceholder: '値を選択',
        },
        sessionBrowser: {
            title: 'セッション',
            emptyTitle: 'セッションなし',
            emptyDescription: 'このエージェントの過去のセッションは見つかりませんでした。',
            active: 'アクティブ',
            load: '読み込み',
            resume: '再開',
            fork: '分岐',
            switchSessionTitle: 'セッションを切り替えますか？',
            switchSessionMessage: 'アクティブなセッションがあります。切り替えると現在のセッションから離れます。',
            sessionCount: ({ count }: { count: number }) => `${count}セッション`,
        },
        agentPicker: {
            title: 'エージェント',
            active: 'アクティブ',
            ...
            statusError: 'エラー',
            agentCount: ({ count }: { count: number }) => `${count}エージェント`,
            switchTitle: 'エージェントを切り替えますか？',
            switchMessage: ({ name }: { name: string }) => `${name}に切り替えますか？現在のセッション状態は保持されます。`,
            switchConfirm: '切り替え',
            switchSuccess: ({ name }: { name: string }) => `${name}に切り替えました`,
            switchFailed: ({ name }: { name: string }) => `${name}への切り替えに失敗しました。前のエージェントに戻しました。`,
            rollbackNotice: 'エージェントの切り替えに失敗しました。前のエージェントに戻しました。',
        },
        permission: {
            title: '許可が必要です',
            expired: 'この許可リクエストは期限切れです',
            rawInput: '生の入力を表示',
            queueCount: ({ count }: { count: number }) => `${count}件保留中`,
            notificationTitle: '許可がリクエストされました',
            notificationBody: ({ tool }: { tool: string }) => `エージェントが実行許可を求めています: ${tool}`,
            historyTitle: '許可履歴',
            historyEmpty: '許可の決定はまだありません',
            outcomeSelected: '承認済み',
            outcomeExpired: '期限切れ',
    ...
    sharing: {
        ...
        title: 'セッションを共有',
        urlSharing: 'リンク共有',
        enableUrlSharing: 'リンク共有を有効にする',
        enableUrlSharingDescription: 'リンクを持っている人は誰でもこのセッションにアクセスできます',
        permission: '権限',
        viewOnly: '閲覧のみ',
        viewAndChat: '閲覧とチャット',
        password: 'パスワード',
        optionalPassword: '任意のパスワード',
        urlCopied: 'リンクをクリップボードにコピーしました',
        shareMessage: 'このコーディングセッションをチェックしてください！',
        shareTitle: 'セッションを共有',
        peopleWithAccess: 'アクセス権を持つ人',
        noShares: 'このセッションにアクセスできる人は他にいません',
        addPeople: '人を追加',
        addPeopleDescription: '連絡先の友達と共有する',
        selectFriend: '友達を選択',
        grantPermission: '権限を付与',
        searchFriends: '友達を検索...',
        noFriendsAvailable: '共有できる友達がいません',
        noMatchingFriends: '一致する友達が見つかりません',
        shareAdded: ({ name }: { name: string }) => `${name}さんと共有しました`,
        shareRemoved: 'アクセス権を削除しました',
        removeAccess: 'アクセス権を削除',
        removeAccessConfirm: ({ name }: { name: string }) => `${name}さんのこのセッションへのアクセス権を削除しますか？`,
        remove: '削除',
        permissionLevel: ({ level }: { level: string }) => `${level}`,
        shareSession: 'セッションを共有',
        shareSessionSubtitle: 'セッションを友達とまたはリンクで共有する',
    },
} as const;
