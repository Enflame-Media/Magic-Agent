...
const barColorClass = computed(() => {
  ...
  if (percentage.value >= 0.6) return "text-amber-500";
  return "text-green-500";
});

const barBgClass = computed(() => {
  if (percentage.value >= 0.85) return "[&>[data-slot=progress-indicator]]:bg-red-500";
  if (percentage.value >= 0.6) return "[&>[data-slot=progress-indicator]]:bg-amber-500";
...
<template>
  <div class="rounded-xl bg-muted/50 p-3 my-1">
    <div class="mb-2 flex items-center gap-1.5">
      ...
      <span class="text-[13px] font-bold" :class="barColorClass"> {{ percentDisplay }}% </span>
    </div>
    <Progress :model-value="percentDisplay" class="mb-2 h-1.5" :class="barBgClass" />
