...
import type { SessionShareUrlConfig, SessionSharePermission } from "@magic-agent/protocol";
...
function handleToggle(enabled: boolean): void {
  ...
  emit("update:config", enabled, localPassword.value || null, localPermission.value);
}

function handlePasswordChange(): void {
  if (!localEnabled.value) return;

  emit("update:config", localEnabled.value, localPassword.value || null, localPermission.value);
}
...
function handlePermissionChange(permission: SessionSharePermission): void {
  ...
  emit("update:config", localEnabled.value, localPassword.value || null, permission);
}
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <Switch :checked="localEnabled" :disabled="isLoading" @update:checked="handleToggle" />
    </div>

    <!-- URL Sharing Content (when enabled) -->
    <div v-if="isEnabled" class="space-y-4 pl-6 border-l-2 border-muted">
      <!-- Shareable URL -->
      <div class="space-y-2">
      ...
      <div class="space-y-2">
        ...
        <div class="relative">
          <Input
            ...
            :placeholder="
              hasPasswordSet
                ? t('sharing.urlSharing.passwordSet')
                : t('sharing.urlSharing.passwordPlaceholder')
            "
            :disabled="isLoading"
            class="pr-10"
