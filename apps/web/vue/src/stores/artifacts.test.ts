...
import { useArtifactsStore, type DecryptedArtifact, type FileTreeNode } from "./artifacts";
...
describe("useArtifactsStore", () => {
  ...
  describe("bulk operations", () => {
    ...
    it("should replace existing artifacts on setArtifacts", () => {
      ...
      store.setArtifacts([createMockArtifact({ id: "new1" }), createMockArtifact({ id: "new2" })]);
  ...
  describe("performance (HAP-873)", () => {
    ...
    it("should handle deeply nested paths", () => {
      ...
      for (let i = 0; i < 10; i++) {
        const depth = "a/b/c/d/e/f/g/h/i/j"
          .split("/")
          .slice(0, i + 1)
          .join("/");
        store.upsertArtifact(
