 ...
 */
export const ALLOWED_COMMANDS: Record<string, readonly string[]> = {
...
export interface CommandValidationResult {
    ...
    /**
     * The reason for blocking (for audit logging)
     */
    reason?: 'metacharacters' | 'not_allowlisted' | 'subcommand_not_allowed';
}
...
export function validateCommand(command: string): CommandValidationResult {
    ...
    if (!(executable in ALLOWED_COMMANDS)) {
        return {
            valid: false,
            error: `Command '${executable}' is not in the allowed commands list. Allowed commands: ${Object.keys(ALLOWED_COMMANDS).join(', ')}`,
            reason: 'not_allowlisted'
        };
    }
    ...
    if (allowedSubcommands.length > 0) {
        ...
        if (!firstArg || !allowedSubcommands.includes(firstArg)) {
            return {
                valid: false,
                error: `Subcommand '${firstArg || '(none)'}' is not allowed for '${executable}'. Allowed: ${allowedSubcommands.join(', ')}`,
 ...
 */
export function validateCommandOrThrow(command: string): void {
 ...
 */
export function getAllowedCommandNames(): string[] {
